...
impl<T: PacketTrait> DnsEncoder<T> {

    pub fn new() -> Self {
        DnsEncoder { _marker: PhantomData }
    }

    pub fn encode(&self, mut packet: T) -> Vec<u8> {
        let mut compress = NamePointerCompress::new();
        let mut data = Self::encode_header(packet.header());
        for question in packet.question().iter() {
            data.extend(Self::encode_question(&question, data.len(), &mut compress))
        }
        for answer in packet.answer().iter() {
    ...
    fn encode_question(question: &T::Question, offset: usize, compress: &mut NamePointerCompress) -> Vec<u8> {
        let mut data = Vec::new();
        data.extend(encode_name(offset, question.domain_name(), compress));
